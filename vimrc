...
function! s:fzf_neighbouring_files()
    let command = $FZF_FILE_COMPLETION
    call fzf#run({
            \  'source': command,
            \  'sink':    'e',
...
function! s:tags()
  ...
  \            '| grep -v -a ^!',
  \ 'options': '+m -d "\t" --with-nth 1,4.. -n 1 --tiebreak=index -1',
  \ 'down':    '40%',
...
"nnoremap <C-]> :Tags<CR>
"nnoremap <C-]> :call <SID>GoToDefinition()<CR>
nnoremap <C-t> <C-o>

map <Char-27>b :FZFBuf<CR>
...
command! FZFFile call s:fzf_neighbouring_files()
...
command! -bang -nargs=* Tags call fzf#vim#tags(expand("<cword>"))

"command! -bang -nargs=* Find call fzf#vim#grep('rg --column --line-number --no-heading --fixed-strings --ignore-case --no-ignore --hidden --follow --glob "!.git/*" --glob "!tags" --glob "!cscope.out" --color "always" '.shellescape(<q-args>), 1, <bang>0)
command! -bang -nargs=* Find call fzf#vim#grep('rg --column --line-number --no-heading --color=always --smart-case '.shellescape(<q-args>), 1, <bang>0 ? fzf#vim#with_preview('up:60%') : fzf#vim#with_preview('right:50%:hidden', '?'), <bang>0)
nmap <Char-27>s :Find <C-R>=expand("<cword>")<CR><CR>
...
noremap <silent> <F10> :call ToggleLineInfo()<CR>
imap <silent> <F10> <c-o>:call ToggleLineInfo()<CR>
...
autocmd BufEnter *.go       setlocal ts=4 sw=4 sts=4 
...
"= Check Symbol
"====================================================
source ~/my-devel-config/plugins/checksymbol.vim
...
" :CocInstall coc-go coc-phpls coc-python coc-html coc-json coc-tsserver coc-pairs
...
map <C-m> :call ToggleBlamerInfo()<CR>
...
if !has("gui_running")
    ...
    noremap <Char-27><F5> :DlvTest<CR>
    noremap <F5> :DlvDebug
    noremap <silent> <F6> :DlvToggleBreakpoint<CR>

    "map <C-g>f :GoTestFunc<CR>
    map gf :GoTestFunc<CR>
    map <C-g>p :GoTest -test.timeout 30s<CR>
    "map <C-g>t :GoAlternate<CR>
    "map gt :GoAlternate<CR>
    map <C-g>c :GoCoverageToggle<CR>
endif
call plug#end()
...
if has("gui_running")
    ...
    set background=dark
    if has("gui_macvim")
        set guifont=Bitstream_Vera_Sans_Mono_newro:h16
    else
